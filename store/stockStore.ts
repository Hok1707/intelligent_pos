...
export const useStockStore = create<StockState>()(
  persist(
    (set, get) => ({
      ...
      isLoading: false,
      lowStockThreshold: 5,
      categories: ['Phone', 'Accessory', 'Part'],
      ...
      addItem: async (newItem) => {
        try {
          ...
          if (response.success) {
            ...
            if (response.data.quantity < threshold) {
              setTimeout(() => {
                useNotificationStore.getState().addNotification({
                  type: 'warning',
      ...
      removeCategory: (category) => {
        const state = get();
        const isUsed = state.items.some(item => item.category === category);
        
        if (isUsed) {
            useNotificationStore.getState().addNotification({
              type: 'error',
              title: 'Cannot Delete Category',
              message: `Category "${category}" is used by existing items.`
            });
            return;
        }
      ...
      renameCategory: (oldName, newName) => {
        set((state) => {
            ...
            const newCategories = state.categories.map(c => c === oldName ? newName : c);
            const newItems = state.items.map(item => item.category === oldName ? { ...item, category: newName } : item);
